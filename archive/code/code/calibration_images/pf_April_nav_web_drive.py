...
from AprilCamera import Camera, TAG_TABLE
    ...
    while True:
        ...
        if not nav.is_running():
            ...
            if tags:
                ...
                last_tag = TAG_TABLE.get(tag.tag_id, str(tag.tag_id))
                ...
                (text_w, text_h), _ = cv2.getTextSize(last_tag, font, font_scale, thickness)
                label_pos = (x_mid - text_w // 2, y_txt + text_h)

                cv2.putText(frame, last_tag, label_pos, font, font_scale, (0, 0, 0), thickness, cv2.LINE_AA)
